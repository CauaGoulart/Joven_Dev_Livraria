...
public class SaleBookResource {
    ...
    @GetMapping("/book/{idBook}")
   	public ResponseEntity<List<SaleBookDTO>> findByBookIgnoreCase(@PathVariable Integer idBook){
    	return ResponseEntity.ok(service.findByBookIgnoreCase(bookService.findById(idBook)).stream().map((saleBook) -> saleBook.toDto()).toList());
           
    }
    
    @GetMapping("/author/{idSale}")
   	public ResponseEntity<List<SaleBookDTO>> findBySaleIgnoreCase(@PathVariable Integer idSale){
    	return ResponseEntity.ok(service.findBySaleIgnoreCase(saleService.findById(idSale)).stream().map((saleBook) -> saleBook.toDto()).toList());
           
    }
    
    @GetMapping("/book-author/{idBook}/{idSale}")
    public ResponseEntity<List<SaleBookDTO>> findByBookAndSaleIgnoreCase(@PathVariable Integer idBook, @PathVariable Integer idSale) {
        return ResponseEntity.ok(service.findByBookAndSaleIgnoreCase(bookService.findById(idBook), saleService.findById(idSale)).stream().map(SaleBook::toDto).toList());
    }
